...
function event(
  opts: Partial<AnalyticsEvent> &
    Pick<AnalyticsEvent, 'ts_ms' | 'workspace_id' | 'viewer_id_key' | 'event_id' | 'deck_id'>,
): AnalyticsEvent {
  return {
    event_name: 'view',
    schema_version: 1,
...
describe('buildSessionEngine', () => {
  ...
  it('closes when the session exceeds maxSessionMs even with heartbeats', () => {
    ...
    const out = engine.apply({
      ...
      viewer_id_key: 'vk-1',
      event: event({
        ts_ms: t1 + (limit / minute + 1) * minute,
        event_id: 'final',
        workspace_id: 'ws-1',
        viewer_id_key: 'vk-1',
        deck_id: 'd1',
    ...
    expect(out.emitted.some((e) => e.type === 'session.ended' && e.reason === 'max_duration')).toBe(
      true,
    );
  });
