...
function event(opts: Partial<AnalyticsEvent> & Pick<AnalyticsEvent, 'ts_ms' | 'event_id' | 'deck_id' | 'workspace_id' | 'viewer_id_key'>): AnalyticsEvent {
  return {
    event_name: 'view',
    schema_version: 1,
...
describe('buildPartitionConsumer', () => {
  it('produces a deterministic session ID across replays', async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      events.push(event({ ts_ms: baseTs + i * 60_000, event_id: `e${i}`, deck_id: 'd1', workspace_id: 'ws-1', viewer_id_key: 'vk-1' }));
    }
    const run = async () => {
      ...
      const consumer = buildPartitionConsumer({
        ...
        onClose: async () => { /* noop */ },
        onEmit: async (_ev: SessionEvent) => { /* noop */ },
      });
  ...
  it('sorts events by ts_ms before applying, so late events get attributed to the right session', async () => {
    ...
    const result = await consumer.run([
      event({ ts_ms: base + 60_000, event_id: 'b', deck_id: 'd1', workspace_id: 'ws-1', viewer_id_key: 'vk-1' }),
      event({ ts_ms: base, event_id: 'a', deck_id: 'd1', workspace_id: 'ws-1', viewer_id_key: 'vk-1' }),
    ]);
  ...
  it('closes a session when a long gap forces a new one', async () => {
    ...
    const result = await consumer.run([
      event({ ts_ms: base, event_id: 'a', deck_id: 'd1', workspace_id: 'ws-1', viewer_id_key: 'vk-1' }),
      event({ ts_ms: base + 31 * 60 * 1000, event_id: 'b', deck_id: 'd1', workspace_id: 'ws-1', viewer_id_key: 'vk-1' }),
    ]);
  ...
  it('counts multiple sessions and multiple workspaces independently', async () => {
    ...
    const base = 1_700_000_000_000;
    const result = await consumer.run([
      event({ ts_ms: base, event_id: '1', deck_id: 'd1', workspace_id: 'ws-1', viewer_id_key: 'vk-1' }),
      event({ ts_ms: base + 60_000, event_id: '2', deck_id: 'd1', workspace_id: 'ws-1', viewer_id_key: 'vk-1' }),
      event({ ts_ms: base, event_id: '3', deck_id: 'd1', workspace_id: 'ws-2', viewer_id_key: 'vk-1' }),
    ]);
    expect(result.started).toBe(2);
