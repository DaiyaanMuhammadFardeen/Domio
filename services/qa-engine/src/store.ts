...
export interface CreateThreadRow { thread: QaThread; }
...
export const notFoundError = (id: string): QaStoreError => makeStoreError('NOT_FOUND', `qa ${id} not found`);
export const conflictError = (id: string, v: number): QaStoreError => makeStoreError('CONFLICT', `qa ${id} concurrency (current ${v})`);
export const tooLongError = (len: number, max: number): QaStoreError => makeStoreError('TOO_LONG', `qa body too long: ${len} > ${max}`);
export const alreadyUpvotedError = (participant_id: string): QaStoreError => makeStoreError('ALREADY_UPVOTED', `participant ${participant_id} already upvoted`);
...
export interface QaStore {
  ...
  listBySession(input: { workspace_id: string; session_id: string; status?: QaThread['status'] }): Promise<ReadonlyArray<QaSubmit>>;
}
