...
describe('Wave 4 — Curated / Brand-Lock', () => {
  ...
  describe('getBrandLock', () => {
    ...
    it('throws for non-existent lock', async () => {
      await expect(service.getBrandLock('ws-1', 'brand-1', 'listing-1')).rejects.toThrow(
        BrandLockNotFoundError,
      );
    });
  });
  ...
  describe('assertBrandLockAllowed', () => {
    ...
    it('throws BrandLockDeniedError for deny lock', async () => {
      ...
      await expect(service.assertBrandLockAllowed('ws-1', 'brand-1', 'listing-1')).rejects.toThrow(
        BrandLockDeniedError,
      );
    });
  });
...
describe('Wave 4 — Takedowns', () => {
  ...
  describe('fileTakedown', () => {
    it('files a takedown request', async () => {
      const listing = await service.getListing((await store.listListings())[0]!.id);
  ...
  describe('reviewTakedown', () => {
    ...
    it('throws for invalid transition', async () => {
      ...
      await expect(service.confirmTakedown(request.id)).rejects.toThrow(
...
describe('Wave 4 — Trust Scoring', () => {
  ...
  describe('computeAndStoreTrustScore', () => {
    ...
    it('throws for non-existent listing', async () => {
      await expect(service.computeAndStoreTrustScore('nonexistent', {})).rejects.toThrow(
        'Listing not found',
      );
    });
  });
...
describe('Wave 4 — Handler integration', () => {
  ...
  describe('reviewTakedown', () => {
    it('returns 200', async () => {
      ...
      const res = await handlers.reviewTakedown(makeReq({ request_id: request.id }, {}), ctx);
      expect(res.status).toBe(200);
    });
  });
  ...
  describe('getTrustScore', () => {
    it('returns 200 with null when no score', async () => {
      const listing = (await store.listListings())[0]!;
      const res = await handlers.getTrustScore(makeReq({ listing_id: listing.id }, {}), ctx);
      expect(res.status).toBe(200);
    ...
    it('returns 200 with dismissed decision', async () => {
      ...
      const res = await handlers.resolveTakedownRequest(
        makeReq(
          ...
          { decision: 'dismissed', resolution_notes: 'Not valid' },
        ),
        ctx,
      );
      expect(res.status).toBe(200);
  ...
  describe('deleteBrandLock', () => {
    it('returns 204 with no body', async () => {
      ...
      const res = await handlers.deleteBrandLock(makeReq({ brand_lock_id: lock.id }, {}), ctx);
