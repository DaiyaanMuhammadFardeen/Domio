...
export class AnnotationService {
  ...
  async commit(
    ...
    const row: AnnotationLayerRecord = {
      id,
      workspace_id: input.workspace_id,
      session_id: input.session_id,
    ...
    let persisted: AnnotationLayerRecord;
    try {
    ...
    await this.audit.emit({
      ...
      ts: now,
      after: describeAnnotationForAudit(persisted) as JsonObject,
      meta: {
        ephemeral,
  ...
  async rollback(input: AnnotationRollbackInput, ctx: { actorId: string }): Promise<void> {
  ...
  async promote(
    ...
    await this.audit.emit({
      ...
      ts: this.clock(),
      after: describeAnnotationForAudit(promoted) as JsonObject,
    });
  ...
  async onSessionEnded(session_id: string, workspace_id: string): Promise<void> {
  ...
  static decodeGeometry(_kind: AnnotationCommitInput['kind'], raw: unknown): AnnotationGeometry {
    return raw as AnnotationGeometry;
  }
}

function mapStoreError(e: unknown): Error {
  if (e && typeof e === 'object' && 'code' in (e as { code?: string })) {
    const code = (e as { code: string }).code;
    if (code === 'NOT_FOUND') return new AnnotationNotFoundError((e as Error).message);
    if (code === 'CONFLICT') return new AnnotationConflictError((e as Error).message);
    if (code === 'IMMUTABLE') return new AnnotationConflictError((e as Error).message);
    if (code === 'ENDED') return new AnnotationNotFoundError((e as Error).message);
  }
  return e as Error;
}
