...
export class PgAnnotationStore {
    ...
    async create(row) {
        return this.withClient(async (c) => {
            const res = await c.query(`INSERT INTO annotation_layer (
    ...
    async getById(id) {
        return this.withClient(async (c) => {
            const res = await c.query(`SELECT id, workspace_id, presenter_session_id AS session_id, slide_id, kind,
                geometry, style, color, stroke_width, ephemeral, saved_overlay_id,
                drawn_by, drawn_by_display_name,
                EXTRACT(EPOCH FROM created_at) * 1000 AS created_at_ms
         FROM annotation_layer WHERE id = $1`, [id]);
    ...
    async listForSession(session_id, ephemeral) {
        return this.withClient(async (c) => {
         ...
         ORDER BY created_at ASC`, [session_id, ephemeral]);
    ...
    async listSavedForSlide(slide_id) {
        return this.withClient(async (c) => {
         ...
         ORDER BY created_at ASC`, [slide_id]);
            return res.rows.map(rowToRecord);
        });
    }
    async rollback(id, workspace_id) {
        return this.withClient(async (c) => {
            const res = await c.query(`DELETE FROM annotation_layer
         WHERE id = $1 AND workspace_id = $2 AND ephemeral = true`, [id, workspace_id]);
            if (res.rowCount === 0) {
                // Either not found or not ephemeral — distinguish.
                const exists = await c.query(`SELECT ephemeral FROM annotation_layer WHERE id = $1 AND workspace_id = $2`, [id, workspace_id]);
                if (exists.rowCount === 0)
                    throw makeStoreError('NOT_FOUND', `annotation ${id} not found`);
    ...
    async promote(id, workspace_id, by) {
        return this.withClient(async (c) => {
            const res = await c.query(`UPDATE annotation_layer
         SET ephemeral = false,
         ...
         RETURNING id, workspace_id, presenter_session_id AS session_id, slide_id, kind,
                   geometry, style, color, stroke_width, ephemeral, saved_overlay_id,
                   drawn_by, drawn_by_display_name,
                   EXTRACT(EPOCH FROM created_at) * 1000 AS created_at_ms`, [id, workspace_id, by]);
            if (res.rowCount === 0)
                throw makeStoreError('NOT_FOUND', `annotation ${id} not found`);
            return rowToRecord(res.rows[0]);
        });
    ...
    async clearEphemeral(session_id, workspace_id) {
        await this.withClient((c) => c.query(`DELETE FROM annotation_layer
         WHERE presenter_session_id = $1 AND workspace_id = $2 AND ephemeral = true`, [session_id, workspace_id]));
    }
}
