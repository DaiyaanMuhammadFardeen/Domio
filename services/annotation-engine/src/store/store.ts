...
export function makeStoreError(code: StoreErrorCode, message: string): StoreError {
  const e = new Error(message) as StoreError;
  (e as { code: StoreErrorCode }).code = code;
  return e;
}

export function isStore(x: unknown): x is AnnotationStore {
