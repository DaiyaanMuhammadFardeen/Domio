...
export class AnnotationService {
    ...
    constructor(opts) {
        if (!isStore(opts.store))
            throw new Error('AnnotationService: store is required');
        if (!opts.audit)
            throw new Error('AnnotationService: audit emitter is required');
    ...
    async commit(input, ctx) {
        ...
        if (input.idempotency_key) {
            const prior = await this.idempotency.get(input.idempotency_key, input.workspace_id, input.session_id);
            if (prior?.response)
...
function mapStoreError(e) {
    if (e && typeof e === 'object' && 'code' in e) {
        const code = e.code;
        if (code === 'NOT_FOUND')
            return new AnnotationNotFoundError(e.message);
        if (code === 'CONFLICT')
            return new AnnotationConflictError(e.message);
        if (code === 'IMMUTABLE')
            return new AnnotationConflictError(e.message);
        if (code === 'ENDED')
            return new AnnotationNotFoundError(e.message);
