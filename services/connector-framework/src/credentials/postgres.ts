/**
 * Credential validation — Postgres (Phase 08).
 */
...
export function validatePostgresCredentials(input: Record<string, unknown>): ValidationResult {
  ...
  return { ok: true, credential_ref: `connectors/${String(input.tenant_id ?? 'default')}/pg_creds` };
}
