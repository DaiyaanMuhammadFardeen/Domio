...
export function validateSnowflakeCredentials(input: Record<string, unknown>): ValidationResult {
  ...
  if (!input.account || typeof input.account !== 'string') errors.push({ field: 'account', message: 'account is required' });
  if (!input.warehouse || typeof input.warehouse !== 'string') errors.push({ field: 'warehouse', message: 'warehouse is required' });
  ...
  return { ok: true, credential_ref: `connectors/${String(input.tenant_id ?? 'default')}/sf_creds` };
}
