...
export function validateBigqueryCredentials(input: Record<string, unknown>): ValidationResult {
  ...
  if (!input.key && !input.service_account_json)
    errors.push({ field: 'key', message: 'key or service_account_json is required' });
  if (errors.length > 0) return { ok: false, errors };
  return {
    ok: true,
    credential_ref: `connectors/${String(input.tenant_id ?? 'default')}/bq_creds`,
  };
}
