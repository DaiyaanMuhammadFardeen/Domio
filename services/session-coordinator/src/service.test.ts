...
import {
  InMemorySessionCoordinatorStore,
...
describe('session-coordinator', () => {
  it('summarises a session from membership rows', async () => {
    ...
    store.upsert(
      makeMembership({ participant_id: participantId('p3'), shard_index: 0, state: 'left' }),
    );
  ...
  it('throws SessionNotFoundError when no rows exist', async () => {
    ...
    await expect(
      service.summarize({ workspace_id: 'w1', session_id: 's-missing' }),
  ...
  it('listMembership filters by since_ms', async () => {
    ...
    const page = await service.listMembership({
      workspace_id: 'w1',
      session_id: 's1',
      since_ms: Date.parse('2026-01-01T00:00:30.000Z'),
    });
  ...
  it('fanoutPlan returns sorted shards and count', async () => {
    ...
    store.upsert(
      makeMembership({ participant_id: participantId('p4'), shard_index: 5, state: 'left' }),
    );
  ...
  it('fanoutPlan returns empty for unknown session', async () => {
    const { service } = buildService();
    await expect(
      service.fanoutPlan({ workspace_id: 'w1', session_id: 's-missing' }),
  ...
  it('rejects listMembership without workspace_id', async () => {
    const { service } = buildService();
    await expect(service.listMembership({ workspace_id: '', session_id: 's1' })).rejects.toThrow(
      /workspace/,
    );
  });
});
