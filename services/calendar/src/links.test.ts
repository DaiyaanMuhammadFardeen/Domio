...
describe('validateCalendarLinkInput', () => {
  ...
  it('throws when deck_id is missing', () => {
    expect(() => validateCalendarLinkInput(makeInput({ deck_id: '' }), 'user-001')).toThrow(
      CalendarValidationError,
    );
  });

  it('throws when user_id is empty', () => {
    expect(() => validateCalendarLinkInput(makeInput(), '')).toThrow(CalendarValidationError);
  });

  it('throws when vendor is invalid', () => {
    expect(() =>
      validateCalendarLinkInput(makeInput({ vendor: 'invalid' as 'google' }), 'user-001'),
  ...
  it('passes for all valid vendors', () => {
    ...
    for (const vendor of vendors) {
      expect(() => validateCalendarLinkInput(makeInput({ vendor }), 'user-001')).not.toThrow();
    }
  });

  it('throws when event_id is empty', () => {
    expect(() => validateCalendarLinkInput(makeInput({ event_id: '' }), 'user-001')).toThrow(
      CalendarValidationError,
    );
  });
  ...
  it('throws when event_start_at is invalid', () => {
    expect(() =>
      validateCalendarLinkInput(makeInput({ event_start_at: 'not-a-date' }), 'user-001'),
    ).toThrow(CalendarValidationError);
  });

  it('throws when is_recurring is true but recurrence_id is missing', () => {
    expect(() =>
      validateCalendarLinkInput(makeInput({ is_recurring: true, recurrence_id: null }), 'user-001'),
    ).toThrow(CalendarValidationError);
  });

  it('throws when is_recurring is true but recurrence_id is empty string', () => {
    expect(() =>
      validateCalendarLinkInput(makeInput({ is_recurring: true, recurrence_id: '' }), 'user-001'),
    ).toThrow(CalendarValidationError);
  });
