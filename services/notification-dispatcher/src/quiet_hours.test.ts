...
describe('quiet_hours', () => {
  describe('isQuietHour', () => {
    ...
    it('returns false for empty window (start === end)', () => {
      ...
      expect(
        isQuietHour({ start: 9, end: 9, tz: 'UTC' }, Date.now(), { offsetMinutes: fixedOffset(0) }),
      ).toBe(false);
    });
    ...
    it('same-day window: quiet during hours', () => {
      ...
      expect(
        isQuietHour({ start: 22, end: 7, tz: 'UTC' }, now, { offsetMinutes: fixedOffset(0) }),
      ).toBe(true);
    });
    ...
    it('daytime window: quiet at 14:00 in 09:00–17:00', () => {
      ...
      expect(
        isQuietHour({ start: 9, end: 17, tz: 'UTC' }, now, { offsetMinutes: fixedOffset(0) }),
      ).toBe(true);
    });
    ...
    it('applies timezone offset', () => {
      ...
      expect(
        isQuietHour({ start: 22, end: 7, tz: 'Australia/Sydney' }, now, {
