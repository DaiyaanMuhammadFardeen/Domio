...
111. AI slide designer — describe a slide ("comparison of our 3 pricing tiers, playful"), get 4 layout options _(M2 — implemented in `services/ai-orchestrator/internal/designer`; HTTP `POST /v1/ai/designer`)_
112. AI redesign — select an ugly slide, get on-brand redesigns preserving content _(M2 — implemented in `services/ai-orchestrator/internal/redesign`; light / full modes with brand-lock + content preservation; HTTP `POST /v1/ai/redesign`)_
113. Copy assistant: shorten to bullet, punch up headlines, fix tone, translate (100+ languages) while preserving layout _(M2 — implemented in `services/ai-orchestrator/internal/copy`; glossary + RTL flip; HTTP `POST /v1/ai/copy`)_
114. AI image generation and background removal built into the canvas _(M2 — implemented in `services/ai-orchestrator/internal/image`; provider chain + 2-layer moderation + provenance; HTTP `POST /v1/ai/image`, `POST /v1/ai/image/{id}/remove-background`)_
