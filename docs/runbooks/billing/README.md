...
Stripe-backed billing; P22 stretch goal.
...
3. Open the matching Grafana dashboard (`/d/billing-overview`).
4. Search structured logs by trace_id for the failing request.
5. Confirm OTel exporter is wired: `grep -r 'billing' packages/observability/dist`.

## Common fixes

- Catalogue marks this as P22 stretch; SLO is aspirational until P22b.
- Webhook ingestion: confirm `billing_webhook_total` advances after a Stripe test event.
- **Restart**: `kubectl rollout restart deployment/billing` (or local equivalent).
