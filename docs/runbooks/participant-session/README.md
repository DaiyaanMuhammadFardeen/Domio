...
3. Open the matching Grafana dashboard (`/d/participant-session-overview`).
4. Search structured logs by trace_id for the failing request.
5. Confirm OTel exporter is wired: `grep -r 'participant-session' packages/observability/dist`.

## Common fixes

- Check `participant_session_count` and `participant_join_ms`.
- Postgres connection pool saturation is the most common cause of p95 drift.
- **Restart**: `kubectl rollout restart deployment/participant-session` (or local equivalent).
