...
import { joinAudience, JoinError } from '@/runtime/join-client';
import { hashFingerprint } from '@/runtime/device-id';

describe('join-client', () => {
  ...
  it('POSTs the join body and returns the parsed response', async () => {
    fetchMock.mockResolvedValueOnce({
      ...
      json: async () => ({
        session_id: 's1',
        bundle: { session_id: 's1', ended_at: null, current_slide_id: 'sl1', presenter_display_name: 'A', title: 'T' },
