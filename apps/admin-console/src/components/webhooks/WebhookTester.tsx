...
const DEFAULT_SAMPLE = JSON.stringify(
  { event: 'custom.event', data: { hello: 'world' } },
  null,
  2,
);
...
export function WebhookTester({ subscriptions, onTested }: WebhookTesterProps) {
  ...
  const [payloadText, setPayloadText] = useState<string>(SAMPLE_PAYLOADS['deck.viewed'] ?? DEFAULT_SAMPLE);
  ...
  return (
    <div
      data-testid="webhooks-tester"
      className="grid grid-cols-1 gap-4 lg:grid-cols-2"
    >
      <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-600">
          <FormattedMessage
            id="admin.webhooks.tester.heading"
            catalogue={CATALOGUE}
          />
        </h3>
        ...
        <label className="block">
          <span className="block text-xs font-medium uppercase tracking-wide text-slate-500">
            <FormattedMessage
              id="admin.webhooks.tester.subscription"
              catalogue={CATALOGUE}
            />
          </span>
          <select
          ...
          >
            {subscriptions.length === 0 && (
              <option value="">— no subscriptions yet —</option>
            )}
            {subscriptions.map((s) => (
        ...
        <div>
          <label className="block">
            <span className="block text-xs font-medium uppercase tracking-wide text-slate-500">
              <FormattedMessage
                id="admin.webhooks.tester.payload"
                catalogue={CATALOGUE}
              />
            </span>
            <textarea
          ...
          <div className="mt-1 flex justify-end">
            ...
            >
              <FormattedMessage
                id="admin.webhooks.tester.useSample"
                catalogue={CATALOGUE}
              />
            </button>
          </div>
      ...
      <div>
        <h3 className="mb-2 text-sm font-semibold uppercase tracking-wide text-slate-600">
          <FormattedMessage
            id="admin.webhooks.tester.result"
            catalogue={CATALOGUE}
          />
        </h3>
