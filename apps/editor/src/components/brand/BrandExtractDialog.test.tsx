...
describe('BrandExtractDialog', () => {
  it('does not render when closed', () => {
    render(
      <BrandExtractDialog open={false} onClose={vi.fn()} onAccept={vi.fn()} />,
    );
  ...
  it('renders when open', () => {
    render(
      <BrandExtractDialog open onClose={vi.fn()} onAccept={vi.fn()} />,
    );
  ...
  it('accepts an extracted kit and emits onAccept with a built kit', async () => {
    ...
    fireEvent.change(screen.getByTestId('brand-extract-url'), { target: { value: 'https://example.com' } });
    fireEvent.click(screen.getByTestId('brand-extract-go'));
    await waitFor(() => {
      expect(screen.getByTestId('brand-extract-result')).toBeInTheDocument();
    });
    ...
    const kit = onAccept.mock.calls[0]?.[0] as { id: string; primaryHex: string; typography: unknown[] };
