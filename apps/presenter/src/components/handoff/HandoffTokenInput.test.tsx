...
describe('HandoffTokenInput', () => {
  ...
  it('disables submit until a valid token is entered', () => {
    render(<HandoffTokenInput sessionId="s1" etag='"v1"' onClose={vi.fn()} onClaim={vi.fn()} />);
    expect((screen.getByTestId('handoff-token-input-submit') as HTMLButtonElement).disabled).toBe(
      true,
    );
  ...
  it('calls onClose when the cancel button is clicked', () => {
    const onClose = vi.fn();
    render(<HandoffTokenInput sessionId="s1" etag='"v1"' onClose={onClose} onClaim={vi.fn()} />);
  ...
  it('submits the token via fetch and calls onClaim on success', async () => {
    const onClaim = vi.fn();
    const onClose = vi.fn();
    render(<HandoffTokenInput sessionId="s1" etag='"v1"' onClose={onClose} onClaim={onClaim} />);
    fireEvent.change(screen.getByTestId('handoff-token-input-field'), {
