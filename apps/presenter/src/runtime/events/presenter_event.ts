...
export function emitPresenterEvent(
  ...
  if (data.co_presenter_user_id !== undefined) event.co_presenter_user_id = data.co_presenter_user_id;
...
export const presenterEmitHelpers = {
  slideAdvance: (ctx: PresenterEmitContext, slideId: string) =>
    emitPresenterEvent(ctx, 'slide_advance', { action_data: JSON.stringify({ slide_id: slideId }) }),
  slideRegress: (ctx: PresenterEmitContext, slideId: string) =>
    emitPresenterEvent(ctx, 'slide_regress', { action_data: JSON.stringify({ slide_id: slideId }) }),
  ...
  sessionStart: (ctx: PresenterEmitContext) =>
    emitPresenterEvent(ctx, 'session_start'),
  sessionPause: (ctx: PresenterEmitContext) =>
    emitPresenterEvent(ctx, 'session_pause'),
  sessionResume: (ctx: PresenterEmitContext) =>
    emitPresenterEvent(ctx, 'session_resume'),
  sessionEnd: (ctx: PresenterEmitContext) =>
    emitPresenterEvent(ctx, 'session_end'),
  hudOpen: (ctx: PresenterEmitContext) =>
    emitPresenterEvent(ctx, 'hud_open'),
  hudClose: (ctx: PresenterEmitContext) =>
    emitPresenterEvent(ctx, 'hud_close'),
  ...
  spotlightOff: (ctx: PresenterEmitContext) =>
    emitPresenterEvent(ctx, 'spotlight_off'),
