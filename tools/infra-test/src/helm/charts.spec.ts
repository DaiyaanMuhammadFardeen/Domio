...
describe('Negative fixture — chart security context cannot regress', () => {
  ...
  it('schema rejects values without required fields', () => {
    const schema = JSON.parse(readText(`${CHART_ROOT}/domio/values.schema.json`)) as Record<
      string,
      unknown
    >;
    expect(JSON.stringify(schema)).toMatch(
      /"runAsNonRoot":\s*\{"type":\s*"boolean",\s*"const":\s*true\}/,
    );
  });
});
