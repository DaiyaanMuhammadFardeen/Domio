...
function expectField(raw: string, field: string): void {
  assert.ok(raw.includes(field), `dependabot.yml is missing required field "${field}"`);
}
...
test('renovate.json schedule is weekday-only', () => {
  ...
  for (const day of ['monday', 'tuesday', 'wednesday', 'thursday', 'friday']) {
    assert.ok(lower.includes(day), `renovate.json schedule must include ${day}`);
  }
...
test('renovate.json has security patches split out and not automerged', () => {
  ...
  const block = raw.slice(securityIdx, nextGroupIdx === -1 ? raw.length : nextGroupIdx);
...
test('renovate.json hostRules use env-var only references (no secrets committed)', () => {
  ...
  for (const r of rules) {
    ...
    if (token) {
      assert.ok(token.includes('env.'), `hostRule token must be env-var only, got: ${token}`);
      assert.ok(
        !token.startsWith('npm_') && !token.startsWith('pypi-') && !token.startsWith('dckr_'),
...
test('dependabot.yml covers npm, gomod, pip, github-actions, docker', () => {
  ...
  for (const e of ['npm', 'gomod', 'pip', 'github-actions', 'docker']) {
    assert.ok(raw.includes(`package-ecosystem: "${e}"`), `missing ecosystem: ${e}`);
  }
});
