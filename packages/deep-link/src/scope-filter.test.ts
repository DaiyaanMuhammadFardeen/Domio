...
const PUBLIC_DECK: DeepLinkVarEntry = {
  name: 'TIER',
  value: 'annual',
  visibility: 'deck_public',
  scope: 'deck',
};
const PRIVATE_SESSION: DeepLinkVarEntry = {
  name: 'NAME',
  value: 'Bear',
  visibility: 'private',
  scope: 'session',
};
const SERVER_ONLY: DeepLinkVarEntry = {
  name: 'SECRET',
  value: 'shh',
  visibility: 'server_only',
  scope: 'deck',
};
const VIEWER_SCOPED: DeepLinkVarEntry = {
  name: 'PIN',
  value: '1234',
  visibility: 'deck_public',
  scope: 'viewer',
...
describe('scopeFilter', () => {
  ...
  it('strips private entries unless authoring viewer matches', () => {
    const out1 = scopeFilter([PRIVATE_SESSION], {
      viewer_scope: 'public',
      requesting_viewer_id: 'u1',
    });
    expect(out1).toHaveLength(0);
    // On a tenant-scoped link, private session vars round-trip
    // when authoring === requesting.
    const out2 = scopeFilter([PRIVATE_SESSION], {
      viewer_scope: 'tenant',
      authoring_viewer_id: 'u1',
      requesting_viewer_id: 'u1',
    });
  ...
  it('private entries are dropped on public links even with matching authoring viewer', () => {
    ...
    const out = scopeFilter([PRIVATE_SESSION], {
      viewer_scope: 'public',
      authoring_viewer_id: 'u1',
      requesting_viewer_id: 'u1',
    });
  ...
  it('keeps session/viewer entries for tenant-scoped links when authoring === requesting', () => {
    const out = scopeFilter([VIEWER_SCOPED], {
      viewer_scope: 'tenant',
      authoring_viewer_id: 'u1',
      requesting_viewer_id: 'u1',
    });
  ...
  it('strips session/viewer entries for tenant-scoped links when authoring !== requesting', () => {
    const out = scopeFilter([VIEWER_SCOPED], {
      viewer_scope: 'tenant',
      authoring_viewer_id: 'u1',
      requesting_viewer_id: 'u2',
    });
    expect(out).toHaveLength(0);
  });

  it('returns an empty list when no entries pass', () => {
    const out = scopeFilter([SERVER_ONLY, PRIVATE_SESSION, VIEWER_SCOPED], {
      viewer_scope: 'public',
      requesting_viewer_id: 'u2',
    });
    expect(out).toHaveLength(0);
