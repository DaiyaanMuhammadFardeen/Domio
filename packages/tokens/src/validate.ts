...
export function validateTokenValue(value: unknown, path = 'value'): ValidationResult {
  ...
  if (value === null || typeof value !== 'object') {
    return {
      valid: false,
      issues: [{ path, message: 'TokenValue must be an object', code: 'TYPE_ERROR' }],
    };
  }
  ...
  if (typeof v.type !== 'string' || !KNOWN_TYPES.has(v.type)) {
    issues.push({
      path: `${path}.type`,
      message: `Unknown token type: ${String(v.type)}`,
      code: 'UNKNOWN_TYPE',
    });
...
export function validateColor(value: unknown, path = 'color'): ValidationResult {
  ...
  if (value === null || typeof value !== 'object') {
    return {
      valid: false,
      issues: [{ path, message: 'TokenColor must be an object', code: 'TYPE_ERROR' }],
    };
  }
  ...
  if (c.space !== 'srgb' && c.space !== 'p3') {
    issues.push({
      path: `${path}.space`,
      message: `Color space must be srgb or p3`,
      code: 'COLOR_SPACE',
    });
  }
  if (!Array.isArray(c.channels) || c.channels.length !== 3) {
    issues.push({
      path: `${path}.channels`,
      message: 'Color channels must be [r, g, b]',
      code: 'CHANNELS_LENGTH',
    });
  } else {
...
export function validateDimension(value: unknown, path = 'dimension'): ValidationResult {
  ...
  if (value === null || typeof value !== 'object') {
    return {
      valid: false,
      issues: [{ path, message: 'TokenDimension must be an object', code: 'TYPE_ERROR' }],
    };
  }
  ...
  if (!['px', 'rem', 'em', '%'].includes(String(d.unit))) {
    issues.push({
      path: `${path}.unit`,
      message: 'Dimension unit must be px, rem, em, or %',
      code: 'DIMENSION_UNIT',
    });
  }
  return { valid: issues.length === 0, issues };
...
export function validateTypography(value: unknown, path = 'typography'): ValidationResult {
  ...
  if (value === null || typeof value !== 'object') {
    return {
      valid: false,
      issues: [{ path, message: 'TokenTypography must be an object', code: 'TYPE_ERROR' }],
    };
  }
  ...
  if (typeof t.fontFamily !== 'string' || t.fontFamily.length === 0) {
    issues.push({
      ...
      message: 'Typography must declare a fontFamily',
  ...
  if (typeof t.fontWeight !== 'number' || t.fontWeight < 100 || t.fontWeight > 900) {
    issues.push({
      ...
      message: 'Font weight must be 100–900',
      code: 'TYPOGRAPHY_WEIGHT',
    });
  }
  if (typeof t.lineHeight !== 'number' || t.lineHeight <= 0) {
    issues.push({
      path: `${path}.lineHeight`,
      message: 'Line height must be > 0',
      code: 'TYPOGRAPHY_LINE_HEIGHT',
    });
  }
  if (!Array.isArray(t.fallbackChain)) {
    issues.push({
      path: `${path}.fallbackChain`,
      message: 'fallbackChain must be an array',
...
export function validateShadow(value: unknown, path = 'shadow'): ValidationResult {
  ...
  if (value === null || typeof value !== 'object') {
    return {
      valid: false,
      issues: [{ path, message: 'TokenShadow must be an object', code: 'TYPE_ERROR' }],
    };
  }
...
export function validateMotion(value: unknown, path = 'motion'): ValidationResult {
  ...
  if (value === null || typeof value !== 'object') {
    return {
      valid: false,
      issues: [{ path, message: 'TokenMotion must be an object', code: 'TYPE_ERROR' }],
    };
  }
  ...
  const td = (v: unknown, p: string) => {
    ...
    if (typeof td.value !== 'number' || !Number.isFinite(td.value) || td.value < 0) {
      issues.push({
        path: `${p}.value`,
        message: 'Duration value must be ≥ 0',
        code: 'TIME_RANGE',
      });
    }
    if (!['ms', 's'].includes(String(td.unit))) {
      issues.push({
        path: `${p}.unit`,
        message: 'Duration unit must be ms or s',
        code: 'TIME_UNIT',
      });
    }
  };
  ...
  if (typeof m.easing !== 'string' || !knownEasings.has(m.easing)) {
    issues.push({
      path: `${path}.easing`,
      message: `Unknown easing: ${String(m.easing)}`,
      code: 'MOTION_EASING',
    });
  }
  return { valid: issues.length === 0, issues };
...
export function validateContent(value: unknown, path = 'content'): ValidationResult {
  ...
  if (value === null || typeof value !== 'object') {
    return {
      valid: false,
      issues: [{ path, message: 'TokenContent must be an object', code: 'TYPE_ERROR' }],
    };
  }
...
export function validateTokenDefinition(input: unknown): ValidationResult {
  ...
  if (input === null || typeof input !== 'object') {
    return {
      valid: false,
      issues: [{ path: '', message: 'Token definition must be an object', code: 'TYPE_ERROR' }],
    };
  }
