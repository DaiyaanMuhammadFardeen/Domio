...
const thresholdsProp: PropSchemaFragment = {
  ...
  items: {
    ...
    properties: {
      ...
      comparator: {
        type: 'string',
        enum: ['lt', 'lte', 'gt', 'gte', 'eq', 'between', 'outside'],
        default: 'gt',
      },
  ...
  return (props, ctx) => {
    ...
    out.push(
      text(ctx, {
        ...
        w: W - 32,
        h: 24,
        content: fitText(chartType, W - 32, 16),
        fontSize: 16,
        color: tokens.text,
        fontWeight: 600,
        semanticId: 'title',
    ...
    if (chartType === 'Gauge') {
      ...
      out.push({
        id: ctx.id(),
        semanticId: ctx.semanticId('gauge-arc'),
        type: 'vector',
        name: 'gauge-arc',
        parentId: null,
      ...
      out.push({
        id: ctx.id(),
        semanticId: ctx.semanticId('radial-ring'),
        type: 'vector',
        name: 'radial-ring',
        parentId: null,
        transform: { x: 0, y: 0, w: 0, h: 0, rotation: 0 },
      ...
      blocks.forEach((b, i) => {
        ...
        out.push(
          text(ctx, {
            ...
            w: b.w - 16,
            h: bh - 12,
            content: fitText(b.label, b.w - 16, 12),
            fontSize: 12,
            color: tokens.background,
            fontWeight: 600,
            semanticId: `block_label_${i}`,
          }),
        );
      });
      ...
      if (chartType === 'Bar') {
        ...
        if (chartType === 'Area') {
          const fillPoints = [
            { x: points[0]!.x, y: cy + ph },
            ...points,
            { x: points[n - 1]!.x, y: cy + ph },
          ];
          const d =
            fillPoints.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ') + ' Z';
          out.push({
            id: ctx.id(),
            semanticId: ctx.semanticId('area-fill'),
            type: 'vector',
            name: 'area-fill',
            parentId: null,
            transform: { x: 0, y: 0, w: 0, h: 0, rotation: 0 },
            paths: [d],
            fill: { type: 'solid', color: { ...toRGBA(accent), a: 0.15 } },
        ...
        out.push({
          id: ctx.id(),
          semanticId: ctx.semanticId('line'),
          type: 'vector',
          name: 'line',
          parentId: null,
          transform: { x: 0, y: 0, w: 0, h: 0, rotation: 0 },
        ...
        if (chartType === 'Scatter') {
          points.forEach((p, i) => {
            out.push({
              id: ctx.id(),
              semanticId: ctx.semanticId(`dot_${i}`),
              type: 'vector',
              name: `dot_${i}`,
              parentId: null,
              transform: { x: 0, y: 0, w: 0, h: 0, rotation: 0 },
    ...
    out.push(
      text(ctx, {
        ...
        w: W - 32,
        h: 20,
        content: `Live ${chartType} — bind a data source`,
        fontSize: 12,
        color: tokens.muted,
        align: 'middle',
        semanticId: 'placeholder-label',
      }),
    );
...
export const LIVE_BAR = makeLiveDef(
  'bar',
  'Live Bar Chart',
  'Bar chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_LINE = makeLiveDef(
  'line',
  'Live Line Chart',
  'Line chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_AREA = makeLiveDef(
  'area',
  'Live Area Chart',
  'Area chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_PIE = makeLiveDef(
  'pie',
  'Live Pie Chart',
  'Pie chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_SCATTER = makeLiveDef(
  'scatter',
  'Live Scatter Plot',
  'Scatter plot bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_FUNNEL = makeLiveDef(
  'funnel',
  'Live Funnel Chart',
  'Funnel chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_SANKEY = makeLiveDef(
  'sankey',
  'Live Sankey Diagram',
  'Sankey diagram bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_TREEMAP = makeLiveDef(
  'treemap',
  'Live Treemap',
  'Treemap bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_HEATMAP = makeLiveDef(
  'heatmap',
  'Live Heatmap',
  'Heatmap bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_WATERFALL = makeLiveDef(
  'waterfall',
  'Live Waterfall Chart',
  'Waterfall chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_GAUGE = makeLiveDef('gauge', 'Live Gauge', 'Gauge bound to a live data source.', {
  w: 400,
  h: 320,
});
export const LIVE_RADAR = makeLiveDef(
  'radar',
  'Live Radar Chart',
  'Radar chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_CANDLESTICK = makeLiveDef(
  'candlestick',
  'Live Candlestick',
  'Candlestick chart bound to a live data source.',
  { w: 640, h: 400 },
);
export const LIVE_BULLET = makeLiveDef(
  'bullet',
  'Live Bullet Chart',
  'Bullet chart bound to a live data source.',
  { w: 640, h: 320 },
);
