...
export const SECTION_HEADER: DomioComponentDef = {
  ...
  propsSchema: {
    ...
    properties: {
      ...
      title: {
        type: 'string',
        title: 'Title',
        default: 'Where we stand',
        'x-domio-prop': { category: 'Content' },
      },
      subtitle: {
        type: 'string',
        title: 'Subtitle',
        default: 'A quick read of the business before the deep dive.',
        'x-domio-prop': { category: 'Content' },
      },
      align: {
        type: 'string',
  ...
  build: (props, ctx) => {
    ...
    return [
      ...
      text(ctx, {
        x: align === 'center' ? (W - 400) / 2 : 0,
        y: 40,
        w: 400,
        h: 20,
        content: fitText(kicker.toUpperCase(), 400, 13),
        fontSize: 13,
        color: accent,
        fontWeight: 600,
        letterSpacing: 2,
        align: align === 'center' ? 'middle' : 'start',
        semanticId: 'kicker',
      }),
      text(ctx, {
        x: tx,
      ...
      ...(subtitle
        ? [
            text(ctx, {
              ...
              w: tw,
              h: 24,
              content: fitText(subtitle, tw, 18),
              fontSize: 18,
              color: tokens.muted,
              align: align === 'center' ? 'middle' : 'start',
              semanticId: 'subtitle',
    ...
    items.forEach((item, i) => {
      ...
      out.push(
        text(ctx, {
          ...
          w: W - cx - 130,
          h: 20,
          content: fitText(item, W - cx - 130, 17),
          fontSize: 17,
          color: tokens.text,
          fontWeight: 500,
          semanticId: `item_${i}`,
  ...
  propsSchema: {
    ...
    properties: {
      items: {
        ...
        items: { type: 'string' },
        default: [
          'Revenue grew 2.4× year over year',
          'Retention stayed above 90%',
          'New markets opened in APAC and LATAM',
        ],
        'x-domio-prop': { category: 'Content', control: 'repeatable' },
      },
      marker: {
        type: 'string',
        enum: ['dot', 'check', 'arrow'],
        default: 'dot',
    ...
    steps.forEach((s, i) => {
      ...
      out.push(
        text(ctx, {
          ...
          w: slot - 16,
          h: 22,
          content: fitText(s.title as string, slot - 16, 18),
          fontSize: 18,
          color: tokens.text,
          align: 'middle',
          fontWeight: 600,
      ...
      if (typeof s.detail === 'string' && s.detail) {
        out.push(
          text(ctx, {
            ...
            w: slot - 32,
            h: 40,
            content: fitText(s.detail, slot - 32, 13),
            fontSize: 13,
            color: tokens.muted,
            align: 'middle',
...
export const CALLOUT: DomioComponentDef = {
  ...
  propsSchema: {
    ...
    properties: {
      ...
      body: {
        type: 'string',
        title: 'Body',
        default: 'Add context here so the team leaves with the same takeaway.',
        'x-domio-prop': { category: 'Content' },
      },
      tone: {
        type: 'string',
  ...
  build: (props, ctx) => {
    ...
    out.push(
      rect(ctx, { x: 0, y: 0, w: W, h: H, radius: 14, fill: soft, semanticId: 'callout_bg' }),
    );
    out.push(
      rect(ctx, { x: 0, y: 0, w: 5, h: H, radius: 3, fill: color, semanticId: 'callout_bar' }),
    );
    if (showIcon) {
      out.push(
        rect(ctx, {
...
export const QUOTE_BLOCK: DomioComponentDef = {
  ...
  build: (props, ctx) => {
    ...
    out.push(
      rect(ctx, { x: 64, y: 176, w: 48, h: 4, radius: 2, fill: accent, semanticId: 'rule' }),
    );
    out.push(
      text(ctx, {
        x: 64,
        ...
        w: W - 96,
        h: 24,
        content: fitText(author, W - 96, 18),
        fontSize: 18,
        color: tokens.text,
        fontWeight: 700,
        semanticId: 'author',
    ...
    labels.forEach((label, i) => {
      ...
      out.push(
        rect(ctx, {
          ...
          h: pillH,
          radius: pillH / 2,
          fill: outline ? tokens.background : accent,
      ...
      out.push(
        text(ctx, {
          ...
          w: tw,
          h: 15,
          content: fitText(label, tw - padX * 2, fontSize),
          fontSize,
          color: outline ? accent : '#FFFFFF',
          align: 'middle',
          fontWeight: 600,
          semanticId: `badge_text_${i}`,
        }),
      );
      x += tw + gap;
    });
