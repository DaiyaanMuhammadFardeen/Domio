...
export const BENTO_GRID: DomioComponentDef = {
  ...
  build: (props, ctx) => {
    ...
    cells.forEach((c, i) => {
      ...
      const accent = typeof c.accent === 'string' ? c.accent : BENTO_ACCENTS[i % BENTO_ACCENTS.length] ?? '#4F46E5';
      out.push(rect(ctx, { x, y, w: cellW, h: cellH, radius: 20, fill: tokens.background, stroke: tokens.border, strokeWidth: 1, semanticId: `cell_${i}` }));
      out.push(rect(ctx, { x: x + 20, y: y + 20, w: 14, h: 14, radius: 4, fill: accent, semanticId: `cell_dot_${i}` }));
      out.push(text(ctx, { x: x + 20, y: y + 54, w: cellW - 40, h: 28, content: fitText(typeof c.title === 'string' ? c.title : 'Metric', cellW - 40, 24), fontSize: 24, color: tokens.text, fontWeight: 700, semanticId: `cell_title_${i}` }));
      if (typeof c.detail === 'string' && c.detail) {
        out.push(text(ctx, { x: x + 20, y: y + 92, w: cellW - 40, h: 20, content: fitText(c.detail, cellW - 40, 14), fontSize: 14, color: tokens.muted, semanticId: `cell_detail_${i}` }));
      }
    });
...
export const KANBAN_BOARD: DomioComponentDef = {
  ...
  build: (props, ctx) => {
    ...
    columns.forEach((col, i) => {
      const x = i * (colW + gap);
      out.push(rect(ctx, { x, y: 0, w: colW, h: H, radius: 14, fill: tokens.surface, semanticId: `col_${i}` }));
      out.push(text(ctx, { x: x + 16, y: 16, w: colW - 32, h: 22, content: fitText(typeof col.title === 'string' ? col.title : 'Column', colW - 32, 16), fontSize: 16, color: tokens.text, fontWeight: 600, semanticId: `col_title_${i}` }));
      const cards = asArray<string>(col.cards, []).slice(0, 4);
      cards.forEach((card, j) => {
        const cy = 52 + j * (cardH + 12);
        out.push(rect(ctx, { x: x + 10, y: cy, w: colW - 20, h: cardH, radius: 10, fill: tokens.background, stroke: tokens.border, strokeWidth: 1, semanticId: `card_${i}_${j}` }));
        out.push(text(ctx, { x: x + 22, y: cy + (cardH - 15) / 2, w: colW - 44, h: 15, content: fitText(card, colW - 44, 13), fontSize: 13, color: tokens.text, fontWeight: 500, semanticId: `card_text_${i}_${j}` }));
      });
    });
...
export const ORG_CHART: DomioComponentDef = {
  ...
  build: (props, ctx) => {
    ...
    const leader = (typeof props.leader === 'object' && props.leader !== null ? props.leader : {}) as Record<string, unknown>;
    ...
    out.push(rect(ctx, { x: leaderX, y: 0, w: leaderW, h: 76, radius: 14, fill: accent, semanticId: 'leader_card' }));
    out.push(text(ctx, { x: leaderX + 16, y: 16, w: leaderW - 32, h: 22, content: fitText(leaderName, leaderW - 32, 16), fontSize: 16, color: '#FFFFFF', fontWeight: 700, semanticId: 'leader_name' }));
    out.push(text(ctx, { x: leaderX + 16, y: 42, w: leaderW - 32, h: 18, content: fitText(leaderRole, leaderW - 32, 13), fontSize: 13, color: '#E0E7FF', semanticId: 'leader_role' }));

    // Connector from leader down to team row.
    out.push(line(ctx, { x1: W / 2, y1: 76, x2: W / 2, y2: 100, stroke: tokens.border, strokeWidth: 2, semanticId: 'spine' }));
    if (n > 1) {
      const startX = colW / 2;
      const endX = (n - 1) * colW + colW / 2;
      out.push(line(ctx, { x1: startX, y1: 100, x2: endX, y2: 100, stroke: tokens.border, strokeWidth: 2, semanticId: 'branch' }));
    }

    teams.forEach((team, i) => {
      const cx = colW * i + colW / 2;
      out.push(line(ctx, { x1: cx, y1: 100, x2: cx, y2: 124, stroke: tokens.border, strokeWidth: 2, semanticId: `drop_${i}` }));
      const teamW = Math.min(colW - 24, 260);
      const teamX = cx - teamW / 2;
      out.push(rect(ctx, { x: teamX, y: 124, w: teamW, h: 44, radius: 10, fill: tokens.surface, semanticId: `team_${i}` }));
      out.push(text(ctx, { x: teamX + 14, y: 136, w: teamW - 28, h: 20, content: fitText(typeof team.name === 'string' ? team.name : 'Team', teamW - 28, 15), fontSize: 15, color: tokens.text, fontWeight: 600, semanticId: `team_name_${i}` }));
      const members = asArray<string>(team.members, []).slice(0, 5);
      members.forEach((member, j) => {
        const my = 178 + j * 24;
        out.push(rect(ctx, { x: teamX, y: my, w: teamW, h: 20, radius: 5, fill: tokens.background, semanticId: `member_bg_${i}_${j}` }));
        out.push(rect(ctx, { x: teamX + 6, y: my + 6, w: 8, h: 8, radius: 4, fill: accent, semanticId: `member_dot_${i}_${j}` }));
        out.push(text(ctx, { x: teamX + 22, y: my + 3, w: teamW - 34, h: 15, content: fitText(member, teamW - 34, 13), fontSize: 13, color: tokens.muted, semanticId: `member_${i}_${j}` }));
      });
    });
