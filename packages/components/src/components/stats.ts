...
function statCardBody(ctx: BuildContext, tokens: VariantTokens, p: StatCardProps): Element[] {
  ...
  // Accent top bar.
  out.push(
    rect(ctx, { x: 0, y: 0, w: 6, h: H, radius: 3, fill: accent, semanticId: 'accent_bar' }),
  );

  if (showIcon) {
    // Decorative accent chip with a glyph-like path (rounded square + dot).
    out.push(
      rect(ctx, { x: 22, y: 22, w: 34, h: 34, radius: 10, fill: accent, semanticId: 'icon_chip' }),
    );
    out.push(
      rect(ctx, { x: 34, y: 34, w: 10, h: 10, radius: 3, fill: '#FFFFFF', semanticId: 'icon_dot' }),
    );
  }
  ...
  out.push(
    text(ctx, {
      ...
      w: W - 44,
      h: 22,
      content: fitText(label.toUpperCase(), W - 44, 13),
      fontSize: 13,
      color: tokens.muted,
      fontWeight: 600,
      letterSpacing: 1.2,
  ...
  out.push(
    text(ctx, {
      ...
      w: W - 44,
      h: 48,
      content: fitText(valueText, W - 44, 40),
      fontSize: 40,
      color: tokens.text,
      fontWeight: 700,
      semanticId: 'value',
  ...
  if (showDelta) {
    ...
    out.push(
      text(ctx, {
        ...
        w: W - 150,
        h: 20,
        content: 'vs last period',
        fontSize: 13,
        color: tokens.muted,
        semanticId: 'delta_caption',
      }),
    );
  }
...
const statCardProps: DomioPropsSchema = {
  ...
  properties: {
    value: {
      ...
      minimum: 0,
      default: 42,
      'x-domio-prop': { category: 'Content', control: 'number' },
    },
    label: {
      type: 'string',
      title: 'Label',
    ...
    deltaValue: {
      ...
      title: 'Delta %',
      default: 12,
      'x-domio-prop': { category: 'Behavior', control: 'stepper', step: 1 },
    },
    showIcon: {
      type: 'boolean',
      title: 'Show icon chip',
      default: true,
      'x-domio-prop': { category: 'Behavior', control: 'toggle' },
    },
  },
  ...
  build: (props, ctx) => {
    ...
    stats.slice(0, count).forEach((stat, i) => {
      ...
      const innerCtx: BuildContext = {
        ...ctx,
        id: ctx.id,
        semanticId: (role) => ctx.semanticId(`stat${i + 1}.${role}`),
      };
      const body = statCardBody(innerCtx, tokenFor(ctx, accentOf(stat)), {
        value: stat.value,
        label: stat.label,
        unit: stat.unit,
        accent: stat.accent,
  ...
  propsSchema: {
    ...
    properties: {
      ...
      caption: {
        type: 'string',
        title: 'Caption',
        default: 'of teams hit their quarterly target',
        maxLength: 80,
        'x-domio-prop': { category: 'Content' },
      },
      accent: { ...accentSchema },
  ...
  propsSchema: {
    ...
    properties: {
      percent: {
        ...
        maximum: 100,
        default: 64,
        'x-domio-prop': { category: 'Content', control: 'slider', step: 1 },
      },
      label: {
        type: 'string',
        title: 'Label',
  ...
  build: (props, ctx) => {
    ...
    return [
      ...
      text(ctx, {
        ...
        w: W - 44,
        h: 18,
        content: fitText(sublabel, W - 44, 13),
        fontSize: 13,
        color: tokens.muted,
        semanticId: 'sublabel',
      }),
